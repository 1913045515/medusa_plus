...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  if (!fileAssetService.isS3Configured()) {
    return res.status(503).json({
      message: "S3 未配置，无法上传文件。请设置 FILE_ASSET_S3_BUCKET、FILE_ASSET_S3_ACCESS_KEY_ID、FILE_ASSET_S3_SECRET_ACCESS_KEY 环境变量。",
    })
  }
